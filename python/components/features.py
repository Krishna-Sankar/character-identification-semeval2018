...
class MentionPairFeatureExtractor(AbstractFeatureExtractor):
    ...
    def extract(self, mention_pair):
        ...
        r1 = match / len(p_tokens) if len(p_tokens) > 0 else 0
        r2 = match / len(c_tokens) if len(c_tokens) > 0 else 0
...
class MentionFeatureExtractor(AbstractFeatureExtractor):
    ...
    def extract(self, mention):
        ...
        next1_sentence = curr_utterance.statements[st_idx + 1] \
            if curr_utterance is not None and st_idx < len(curr_utterance.statements)-1 else None
        next2_sentence = curr_utterance.statements[st_idx + 2] \
            if curr_utterance is not None and st_idx+1 < len(curr_utterance.statements)-1 else None
        ...
        emb3.append(self.get_tokens_word_vector(curr_utterance.statements[st_idx], 0, len(curr_utterance.statements[st_idx]))
                    if st_idx > 0 and st_idx < len(curr_utterance.statements[st_idx]) else np.zeros(self.word2vec_dim))
        ...
        features.append(self.get_speaker_vector(curr_speaker))        # Current speaker information of the utterance
    ...
    def get_tokens_gender_vector(self, mention):
        ...
        return gvector / float(len(mention.tokens)) if len(mention.tokens) > 0 else np.zeros(self.word2gender_dim)
